...
void tokenize_equal_add(char* cmdline);
...
int main(){
	...
	if(chk_pipe(ttl_list[i])==0){
		char* f1=strchr(ttl_list[i],'=');  //create variable
		if((f1||ttl_list[i][0]=='$')&&ttl_list[i][0]!= '='){
		tokenize_equal_add(ttl_list[i]);
		}
		else{	//actual work
...
void tokenize_equal_add(char* cmdline){
	...
	char *ca;
	char *s;
	printf("%s output",cmdline);
	sleep(5);
	strncpy(ca,cmdline+1,3);
	ca[3]='\0';
